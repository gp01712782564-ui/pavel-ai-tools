...
import { Send, Bot, User, Sparkles, X, FileSearch, Bug, FileText, Globe, Image as ImageIcon } from 'lucide-react';
...
export const AIChatPanel: React.FC<AIChatPanelProps> = ({ messages, onSendMessage, onDebugProject, onGenerateImage, isLoading, onClose, activeFileName }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-[#0d1117] border-l border-gray-200 dark:border-gray-800 shadow-xl w-full transition-colors">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
        {messages.length === 0 && !isImgGenMode && (
          ...
          <form onSubmit={handleImageSubmit} className="p-3 border-t border-gray-200 dark:border-gray-800 bg-amber-50 dark:bg-[#161b22]">
              <div className="flex flex-col space-y-2">
                  <div className="flex justify-between items-center text-xs font-semibold text-amber-600 dark:text-amber-500">
                      ...
                      <button onClick={() => setIsImgGenMode(false)} type="button" className="text-gray-500 hover:text-gray-800 dark:hover:text-gray-300"><X size={14}/></button>
                  </div>
                  <textarea
                    value={imgPrompt}
